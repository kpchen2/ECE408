    ...
    int Batch, int Map_out, int Channel, int Height, int Width, int K)
{
    ...
    for (int tileId = 0; tileId < (unroll_rows + TILE_WIDTH - 1) / TILE_WIDTH; tileId++) {
        int tiled_index_A = tileId * TILE_WIDTH + tx;
        int tiled_index_B = tileId * TILE_WIDTH + ty;

        tileA[ty][tx] = (row < Map_out && tiled_index_A < unroll_rows) ? mask[row * unroll_rows + tiled_index_A] : 0.0f;

        if (col < unroll_cols && tiled_index_B < unroll_rows) {
            int c = tiled_index_B / (K * K);
            int p = (tiled_index_B / K) % K;
            int q = tiled_index_B % K;
            int h = h_out + p;
            int w = w_out + q;
...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float * __restrict__ host_output, const float * __restrict__ host_input, const float * __restrict__ host_mask, float ** __restrict__ device_output_ptr, float ** __restrict__ device_input_ptr, float ** __restrict__ device_mask_ptr, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
...
__host__ void GPUInterface::conv_forward_gpu_epilog(float * __restrict__ host_output, float * __restrict__ device_output, float * __restrict__ device_input, float * __restrict__ device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    const int h_out = Height - K + 1;
    const int w_out = Width - K + 1;
