...
// swept tile width: 4,8,12,16,20,24,32,64
    ...
    int Batch, int Map_out, int Channel, int Height, int Width, int K)
{
    ...
    for (int tileId = 0; tileId < numTiles; tileId++) {
        ...
        tileA[ty][tx] = (row < Map_out && tiledA < unroll_rows) ? mask[row * unroll_rows + tiledA] : 0.0f;

        if (col < unroll_cols && tiledB < unroll_rows) {
            int c = tiledB / (K*K);
            int p = (tiledB / K) % K;
            int q = tiledB % K;
            int h = h_out + p;
        ...
        // swept unrolling depth: 1,2,4,6,8,10
